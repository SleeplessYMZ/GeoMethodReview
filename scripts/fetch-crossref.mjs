...
const USER_AGENT = process.env.CROSSREF_USER_AGENT || "GeoMethodReview/1.0 (mailto:research@example.com)";
const EXPECTED_GROUPS = ["method_family", "data_source", "spatiotemporal_context"];
const REQUIRED_GROUPS = ["method_family"];
const CSV_HEADERS = [
  "month",
  "journal_category",
  "journal",
  ...
  "doi_url",
  "abstract_available",
  "method_family_topics",
  "method_family_keywords",
  "data_source_topics",
  "data_source_keywords",
  "spatiotemporal_context_topics",
  "spatiotemporal_context_keywords",
  "screening_rule",
];
...
function parseKeywordsYaml(text) {
  ...
  const missing = EXPECTED_GROUPS.filter((groupId) => !configured.has(groupId));
    ...
    for (const [index, journal] of journals.entries()) {
      ...
      for (const filter of filters) {
        ...
        for (const item of await fetchItems(journal.issn, filter)) {
          ...
          const methodTopics = groups.method_family || [];
          const dataTopics = groups.data_source || [];
          const contextTopics = groups.spatiotemporal_context || [];
          works.set(doi, [
            month,
            journal.category,
            journal.name,
            ...
            joinTopicNames(dataTopics),
            joinKeywords(dataTopics),
            joinTopicNames(contextTopics),
            joinKeywords(contextTopics),
            "Must match at least one method_family keyword in Crossref title or abstract",
          ]);
        }
      }
